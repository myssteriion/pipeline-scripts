...
public class Pipeline {
	...
	private void init() throws IOException {
		...
		try (InputStream is = getClass().getResourceAsStream("/" + ConstantTools.APPLICATION_PROP_FILE)) {
			application.load(is);
		}
		...
		try (InputStream is = getClass().getResourceAsStream("/" + ConstantTools.PARAM_PROP_FILE)) {
			parameters.load(is);
		}
		...
		try (InputStream is = getClass().getResourceAsStream("/" + ConstantTools.ENV_PROP_FILE)) {
			environment.load(is);
		}
		expectedKeys = new HashSet<String>();
		...
		try (InputStream is = getClass().getResourceAsStream("/" + ConstantTools.TOOLS_PROP_FILE)) {
			tools.load(is);
		}
		expectedKeys = new HashSet<String>();
